...
// reconciler's problem, and `tpctl status` is how you watch it. That difference
// from `tpctl apply` is the entire lesson of challenge 3.
func syncCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if len(specs) == 0 {
				fmt.Printf("no specs in %s/ -- try `tpctl new`\n", dir)
				return nil
			}
			...
			fmt.Println("  tpctl status <name>    what the loop currently believes")
			return nil
		},
	}
